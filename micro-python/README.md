...
1. Copy the example environment file:
   ```bash
   cp backend/stack.env.example backend/stack.env
   ```
...
**IaC Features:**
- **Automated Provisioning:** Grafana automatically connects to InfluxDB on startup using provisioning generated by `backend/docker-compose.yml`.
- **Dashboard-as-Code:** The "Temperature Monitor" dashboard JSON is generated at container startup by `backend/docker-compose.yml`, so it persists across container restarts.
